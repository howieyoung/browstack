...
const agentPlist = (agentLabel, scriptFile, calendarXml, logFile) => `<?xml version="1.0" encoding="UTF-8"?>
...
<dict>
  ...
  <array>
    ...
    <string>${path.join(repoRoot, "scripts", scriptFile)}</string>
  </array>
...
const weeklyPlistPath = installAgent(label, agentPlist(label, "weekly.mjs", weeklyCalendar, "weekly.log"));
installAgent(heartbeatLabel, agentPlist(heartbeatLabel, "heartbeat.mjs", heartbeatCalendar, "heartbeat.log"));

const dayNames = ["日", "一", "二", "三", "四", "五", "六"];
const hh = (h) => String(h).padStart(2, "0");
console.log(`已排程 / Scheduled: 每週${dayNames[day]} ${hh(hour)}:${hh(minute)} 自動出刊（${hh(retryHour)}:${hh(minute)} 當日重試，成功則自動跳過）`);
console.log(`憑證心跳 / heartbeat: 每天 09:37 保鮮 Claude CLI 憑證，失效即通知`);
console.log(`plist: ${weeklyPlistPath}`);
console.log(`日誌 / logs: ${path.join(logDir, "weekly.log")}、${path.join(logDir, "heartbeat.log")}`);
console.log(`解除排程 / uninstall: launchctl bootout gui/$UID/${label} && launchctl bootout gui/$UID/${heartbeatLabel} && rm '${weeklyPlistPath}' '${path.join(laDir, heartbeatLabel + ".plist")}'`);
