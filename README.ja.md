...
すべての号に番号が付き、保存されます：№0 はプレビュー号、№1 は創刊号、以降は第 N 号。`send` の成功が現在の号を封緘し、次の実行は自動的に新しい号を新しい表紙で開きます。成果物は `out/`（号ごとの Web 版＋メール版）と `assets/covers/`（号ごとに 1 枚の表紙）に蓄積され、`out/index.html` で閲覧可能なアーカイブになります。ある週の表紙レンダリングが失敗しても、前号の表紙が再利用されます。
